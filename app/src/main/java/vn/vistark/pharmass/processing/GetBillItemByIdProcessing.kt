...
class GetBillItemByIdProcessing(
    ...
    var onFinished: ((SimpleBillItem?) -> Unit)? = null

    init {
        APIUtils.mAPIServices?.getBillItemById(id)
            ?.enqueue(object : Callback<List<SimpleBillItem>> {
                override fun onFailure(call: Call<List<SimpleBillItem>>, t: Throwable) {
                    DialogNotify.error(
                        context,
                ...
                override fun onResponse(
                    call: Call<List<SimpleBillItem>>,
                    response: Response<List<SimpleBillItem>>
                ) {
