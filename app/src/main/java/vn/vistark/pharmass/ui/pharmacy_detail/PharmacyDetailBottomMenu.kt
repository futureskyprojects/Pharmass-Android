...
import vn.vistark.pharmass.ui.pharmacy_detail.fragments.MiniMapPharmacyFragment
        ...
        fun all(pharmacy: Pharmacy): Array<PharmacyDetailBottomMenu> {
            return arrayOf(
                PharmacyDetailBottomMenu(
                    "Giới thiệu",
                    IntroductionFragment.newInstance(pharmacy.introduction)
                ),
                PharmacyDetailBottomMenu(
                    "Bản đồ",
                    MiniMapPharmacyFragment.newInstance(Gson().toJson(pharmacy.coordinates))
                )
