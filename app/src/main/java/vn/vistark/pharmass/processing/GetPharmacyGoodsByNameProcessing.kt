...
class GetPharmacyGoodsByNameProcessing(
    context: Context,
    pharmacyId: Int,
    name: String
) {
    ...
    init {
        APIUtils.mAPIServices?.getPharmacyGoodsByName(pharmacyId, name)
            ?.enqueue(object : Callback<List<Goods>> {
                override fun onFailure(call: Call<List<Goods>>, t: Throwable) {
                    DialogNotify.error(
                        context,
                        t.message ?: "Lỗi không xác định khi lấy danh sách hàng theo tên"
                    )
                }
