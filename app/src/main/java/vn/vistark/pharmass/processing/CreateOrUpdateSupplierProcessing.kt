...
class CreateOrUpdateSupplierProcessing(
    context: Context,
    supplier: Supplier
) {
    var onFinished: ((Supplier?) -> Unit)? = null

    init {
        val loading = DialogNotify.loading(context)
        loading.show("Đang tiến hành thêm nhà cung cấp")
        APIUtils.mAPIServices?.run {
            if (supplier.id > 0) {
                updateSupplierOfPharmacy(supplier.id, supplier)
            } else {
                createSupplierOfPharmacy(supplier)
            }
        }
