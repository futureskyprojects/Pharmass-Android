...
class CreatePharmacyStaffProcessing(
    context: Context,
    simplePharmacyStaff: SimplePharmacyStaff
) {
    var onFinished: ((PharmacyStaff?) -> Unit)? = null

    init {
        val loading = DialogNotify.loading(context)
        loading.show("Đang cập nhân viên bị thiếu của nhà thuốc")
        APIUtils.mAPIServices?.createPharmacyStaffs(simplePharmacyStaff)
            ?.enqueue(object : Callback<PharmacyStaff> {
                override fun onFailure(call: Call<PharmacyStaff>, t: Throwable) {
                    loading.close()
                    DialogNotify.error(context, t.message ?: "Lỗi không xác định")
                }

                override fun onResponse(
                    call: Call<PharmacyStaff>,
                    response: Response<PharmacyStaff>
                ) {
                    loading.close()
