...
class GetMedicineCategoryByNameProcessing(
    context: Context,
    name: String
) {
    var onFinished: ((List<MedicineCategory>?) -> Unit)? = null

    init {
        APIUtils.mAPIServices?.findMedicineCategoryByName(name)
            ?.enqueue(object : Callback<List<MedicineCategory>> {
                override fun onFailure(call: Call<List<MedicineCategory>>, t: Throwable) {
                    DialogNotify.error(
                        context,
                        t.message ?: "Lỗi không xác định khi lấy danh mục thuốc"
                    )
                }

                override fun onResponse(
                    call: Call<List<MedicineCategory>>,
                    response: Response<List<MedicineCategory>>
                ) {
