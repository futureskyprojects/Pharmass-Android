...
class GetPharmacyGoodsCategoryProcessing(
    val context: Context,
    val pharmacyId: Int
) {
    var onFinished: ((List<GoodsCategory>?) -> Unit)? = null

    var categories: List<GoodsCategory>? = emptyList()

    init {
        getDefaultCategories()
    }

    private fun getDefaultCategories() {
        APIUtils.mAPIServices?.getDefaultGoodsCategory()
            ?.enqueue(object : Callback<List<GoodsCategory>> {
                override fun onFailure(call: Call<List<GoodsCategory>>, t: Throwable) {
                    DialogNotify.error(
                        context,
                        t.message ?: "Lỗi không xác định khi lấy danh sách danh mục hàng"
                    )
                }

                override fun onResponse(
                    call: Call<List<GoodsCategory>>,
                    response: Response<List<GoodsCategory>>
                ) {
    ...
    private fun getPharmacyCategories() {
        APIUtils.mAPIServices?.getPharmacyGoodsCategory(pharmacyId)
            ?.enqueue(object : Callback<List<GoodsCategory>> {
                override fun onFailure(call: Call<List<GoodsCategory>>, t: Throwable) {
                    DialogNotify.error(
                        context,
                ...
                override fun onResponse(
                    ...
                    if (response.isSuccessful) {
                        // Khi thực hiện thành công
                        categories =
                            categories?.plus(response.body()?.toTypedArray() ?: emptyArray())
                        onFinished?.invoke(categories)
                        return
                    } else if (response.code() == 400) {
                        try {
                            // Khi nhận được thông báo lỗi 400
